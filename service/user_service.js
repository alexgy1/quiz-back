...
const CLIEND_ID_ADMIN = config.get("auth.adminclientid");
  ...
  verifyToken: async function (token, isAdmin) {
    ...
    try {
      ...
      const user = await getJSON(
        ORANGE_AUTH_TOKEN_VERIFICATION_URL + token,
        "GET",
        {
          "client-id": isAdmin ? CLIEND_ID_ADMIN : CLIENT_ID,
      ...
      if (error.code != 403) {
        console.error(
          "FATAL verification interface error, please check the code"
        );
      }
      return false;
    }
